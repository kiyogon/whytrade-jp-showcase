...
診断で終わらず、その戦略が過去に「いつ買い、いつ売り、資産がどう動いたか」をタイムマシンで再生できます。診断結果は共有URLでシェアでき、URLに名前や出生データは含まれません。
...
この工程で、固定500銘柄の過去データに対して**10,718回の損益計算**を実行し、結果を保存しました。
...
| 投資スタイル | 12種類 |
| 損益計算 | **10,718回** |
| Devinが担当した計算 | **9,703回（約90.5%）** |
| 後半2年がプラス | **10戦略** |
| 後半2年がマイナス | **2戦略** |
| 後半2年の1位 | **王道トレンド型、+50.6万円** |

発表当日の朝には「直近2年で日経平均に勝てる戦略を作れるか」を締切ぎりぎりまで実験しました。基準の事前登録つきで挑み、全て負けた記録ごと実装リポジトリの `docs/rounds/round5/NIKKEI_CHALLENGE.md` に残しています。

「後半2年」は検証期間（2024-07-26〜2026-07-24）の値です。サービス画面は条件を固定したまま基準日2026-08-14まで再計算して表示するため、金額がわずかに異なります（例: 王道トレンド型は画面では +51.3万円）。

手数料は1注文115円、価格のずれは0.1%として計算しています。

発表当日の朝には「直近2年で日経平均に勝てる戦略を作れるか」を締切ぎりぎりまで実験しました。基準の事前登録つきで挑み、**全て負けた記録ごと**[docs/rounds/round5/NIKKEI_CHALLENGE.md](docs/rounds/round5/NIKKEI_CHALLENGE.md)に残しています。
...
複数のDevinが、全10,718回のうち**9,703回（約90.5%）**を担当しました。最終夜のRound 4では12体のDevinが並行で7,200回を一晩で完走し、発表当日朝のRound 5（日経チャレンジ）でも4体が1,200回を追加しています。
...
Round 4（12スタイル×600、計7,200回）とRound 5のグリッド4本（計1,200回）もDevinの実行です。1,303 + 7,200 + 1,200 = **9,703回**。

9,703 ÷ 10,718 = 90.5%です。残りは第1陣4戦略のRound 1〜3（634回）と、後半2年の答え合わせ・発表用の再計算（Fable実行）です。
...
- 10,718回の段階別内訳とDevin担当9,703回のスタイル別内訳: 「AIに『勝てる戦略を探して』と頼まなかった」「Devinの活用」
